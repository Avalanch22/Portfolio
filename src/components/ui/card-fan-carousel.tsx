...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  const chevron = (direction: "left" | "right") => {
    ...
    return <Icon className="relative z-[2] w-4 h-4 md:w-5 md:h-5" strokeWidth={2.5} />;
  };

  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        {/* We add h-[500px] so the absolute children have space, otherwise it collapses */}
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[400px] md:h-[500px]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-xl bg-white shadow-xl dark:bg-zinc-900 border border-black/10 dark:border-white/10">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
              </div>
            );
            
            // Standardizing card size
            const cardClasses = "fan-card absolute w-[160px] h-[240px] sm:w-[200px] sm:h-[300px] md:w-[240px] md:h-[360px] cursor-pointer will-change-transform";

            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className={`${cardClasses} block`}>{image}</a>
            ) : (
